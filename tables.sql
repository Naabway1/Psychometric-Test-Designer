...
-- Таблица токенов
CREATE TABLE tokens_for_groups IF NOT EXISTS (
...
-- Таблица шкал
CREATE TABLE scales IF NOT EXISTS (
    scale_id SERIAL PRIMARY KEY,
    name VARCHAR(100),
    description TEXT
);
-- Метрики каждого пользователя
CREATE TABLE user_metrics IF NOT EXISTS (
...
-- Связь: вопрос -> шкала
CREATE TABLE question_scale IF NOT EXISTS (
    question_id INT REFERENCES questions(question_id) ON DELETE CASCADE,
...
CREATE TABLE test_scale_metric IF NOT EXISTS (
    test_id INT REFERENCES tests(test_id) ON DELETE CASCADE,
    scale_id INT REFERENCES scales(scale_id) ON DELETE CASCADE,
    metric_id INT REFERENCES metrics(metric_id) ON DELETE CASCADE,
    weight NUMERIC NOT NULL,
...
CREATE TABLE user_scale_results IF NOT EXISTS (
...
CREATE TABLE user_metric_snapshots IF NOT EXISTS (
